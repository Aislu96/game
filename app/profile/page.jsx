    ...
    return (
        <div className="h-screen bg-customFon relative flex flex-col ">
            ...
            <div className="flex flex-col mx-[30px] gap-5 relative overflow-hidden">
                ...
                {isFormVisible && (<div
                    className="opacity-100 py-3 px-1.5 flex flex-col gap-1.5 absolute left-0 top-0 w-full z-20 bg-customFon2 border-[1px] border-solid border-customBorder rounded-xl">
                    <p className="text-medium text-base text-white text-center">Enter your wallet</p>
                    {isFormSubmitted ? (
                        <div
                            className="w-full bg-customFon px-2.5 py-4 border-[1px] border-solid border-customBorder rounded-xl">
                            <p className="text-medium text-base text-white text-center">Your wallet is saved</p>
                        </div>
                    ) : (
                        <form className="flex flex-col gap-1.5" name="form-wallet" onSubmit={handleSubmit}>
                            <input
                                type="text"
                            ...
                            <button
                                type="submit"
                                className={`bg-customYellow2 w-full h-8 rounded-xl text-medium text-base ${!isWalletValid ? "opacity-50" : ""}`}
                ...
                <div className="cursor-pointer flex items-center justify-center h-9 w-full rounded-[10px] border-solid border-[1px] border-transparent gradient-profile-three"
                     onClick={toggleFormVisibility}>
                    <p className="text-medium text-base text-white">
                        {isWalletAttached ? "Change the wallet address" : "Attach a crypto wallet"}
                    </p>
                </div>
                <Link href="/"
                ...
                <div
                    ...
                    <Link href="/" target="_blank"><Image src="unix.svg" alt="unix" width={21} height={21}
                                                          className="object-contain"/></Link>
                    <div className="w-px h-5 bg-customYellow"></div>
                    <Link href="/" target="_blank"><Image src="instagram.svg" alt="instagram" width={21} height={21}
                                                          className="object-contain"/></Link>
                    <div className="w-px h-5 bg-customYellow"></div>
                    <Link href="/" target="_blank"><Image src="facebook.svg" alt="facebook" width={21} height={21}
                                                          className="object-contain"/></Link>
                </div>
            </div>
